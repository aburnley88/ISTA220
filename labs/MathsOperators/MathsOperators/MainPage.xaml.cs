...
namespace MathsOperators
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        {
            int lhs = int.Parse(lhsOperand.Text);//looks at text property of lhsOperand object and converts string to int
            int rhs = int.Parse(rhsOperand.Text);
            int outcome = 0;//declares variable outcome and assigns value of 0 type int
            // TODO: Add rhs to lhs and store the result in outcome
            outcome = lhs + rhs;//assigns the sum of the expression to variable outcome
            expression.Text = $"{lhsOperand.Text} + {rhsOperand.Text}";//creates a literal string 
            result.Text = outcome.ToString();//Text is a property of the result object
        }
